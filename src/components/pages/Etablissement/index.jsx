...
import { HiLocationMarker, HiArrowRight, HiHeart } from "react-icons/hi";
...
function Etablissement({ title }) {
  ...
  useEffect(() => {
    ...
    const isAdminFromUserName =
      user?.name === "Admin Groupe8" || user?.username === "Admin Groupe8";

    // Combiner toutes les méthodes
    const adminStatus =
      isAdminFromLocalStorage || isAdminFromContext || isAdminFromUserName;
  ...
  const fallbackEtablissements = [
    {
      ...
      type: "Restaurant",
      description:
        "Cuisine française raffinée dans un cadre élégant. Spécialités de poisson et de viande avec des produits locaux.",
    ...
    },
    {
      ...
      description:
        "Authentique cuisine italienne avec pâtes fraîches faites maison. Vaste sélection de vins italiens.",
    ...
    },
    {
      ...
      description:
        "Sushi et sashimi préparés par un chef japonais expérimenté. Ambiance zen et apaisante.",
    ...
    },
    {
      ...
      type: "Bistrot",
      description:
        "Bistrot traditionnel avec une touche moderne. Plats généreux et ambiance chaleureuse.",
    ...
    },
    {
      ...
      type: "Café",
      description:
        "Café artisanal et pâtisseries maison. Lieu idéal pour travailler ou se détendre.",
    ...
    },
    {
      ...
      type: "Fast-Food",
      description:
        "Burgers gourmets avec ingrédients de qualité. Options végétariennes disponibles.",
    ...
    },
    {
      ...
      type: "Restaurant",
      description:
        "Cuisine du marché avec produits frais et de saison. Menu changeant quotidiennement.",
    ...
    },
    {
      ...
      type: "Bistrot",
      description:
        "Cuisine lyonnaise traditionnelle dans une ambiance conviviale. Spécialités de charcuterie.",
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (res.data && Array.isArray(res.data)) {
            ...
            res.data &&
            res.data.etablissements &&
            Array.isArray(res.data.etablissements)
          ) {
          ...
          if (!Array.isArray(data)) {
            console.warn(
              "⚠️ Les données de l'API ne sont pas un tableau:",
              data,
            );
            data = [];
          }
          ...
          const processEtabs = async () => {
            ...
            for (const etab of data) {
              try {
                ...
                console.error(
                  "❌ Erreur lors du traitement d'un établissement:",
                  error,
                );
              }
            }
        ...
        if (isMounted) {
          setError(
            `L'API n'est pas disponible (${err.message}). Affichage des données de démonstration.`,
          );
    ...
    const fetchEtabImage = async (etabId) => {
      try {
        const response = await api.get(
          `/groupe-8/etablissements/${etabId}/images`,
        );
        ...
        console.warn(
          `⚠️ Impossible de charger l'image pour ${etabId}:`,
          error.message,
        );
      }
  ...
  const toggleFavorite = useCallback(
    (etabId) => {
      ...
      const favBtn = document.querySelector(
        `.fav-btn-overlay[data-id="${etabId}"]`,
      );
      if (favBtn) {
  ...
  const handleDelete = async (id) => {
    ...
    if (
      !window.confirm(
        "⚠️ Êtes-vous sûr de vouloir supprimer cet établissement ? Cette action est irréversible.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `⚠️ Impossible de supprimer les images: ${imgError.message}`,
        );
      }
      ...
      if (status === 403) {
        ...
        userMessage =
          "Erreur 401 : Non authentifié. Veuillez vous reconnecter.";
      } else if (status === 404) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (etablissements === fallbackEtablissements) {
        ...
        setEtabsWithImages((prev) =>
          prev.map((item) =>
            item.id === currentEtab.id
              ? { ...item, ...editFormData, image: item.image }
              : item,
          ),
        );
      ...
      await api.put(
        `/groupe-8/admin/etablissements/${currentEtab.id}`,
        updateData,
        { headers },
      );
          ...
          {!filtered || filtered.length === 0 ? (
            ...
            <div className="cards-container">
              {filtered.map((etab) => {
                ...
                const isFavorite =
                  Array.isArray(favorites) && favorites.includes(etab.id);
                const currentRating =
                  etab.note || etab.moyenne || etab.rating || 3.5;
                const imageUrl =
                  coverImages[etab.id] || etab.image || etabImages[0];
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 z-20">
                      ...
                      >
                        <HiHeart
                          className={`w-4 h-4 ${isFavorite ? "text-white" : ""}`}
                        />
                      </IconButton>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                      ...
                      >
                        <h3 className="text-xl font-extrabold mb-1 line-clamp-1">
                        ...
                        <div className="mt-3 inline-flex items-center gap-2 text-emerald-300 hover:text-emerald-200 font-semibold text-sm transition-colors">
                          Voir les détails <HiArrowRight className="w-4 h-4" />
                        </div>
                      </Link>
            ...
            <form onSubmit={handleUpdate}>
              ...
              <div className="field">
                ...
                >
                  ...
                  <option value="Restaurant Japonais">
                    Restaurant Japonais
                  </option>
