...
import { HiHeart, HiLocationMarker, HiExclamationCircle, HiArrowRight, HiStar } from "react-icons/hi";
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 md:py-12">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-slate-600 dark:text-slate-400">Chargement de vos favoris...</p>
          </div>
          ...
          <div className="max-w-md mx-auto text-center py-16 px-6 rounded-2xl bg-white dark:bg-slate-800 shadow-xl border border-slate-200 dark:border-slate-700">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Aucun favori pour le moment</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-8">
              Vous n&apos;avez pas encore ajouté d&apos;établissement à vos favoris. Parcourez nos établissements et ajoutez vos préférés !
            </p>
            <button onClick={() => navigate("/etablissements")} className="inline-flex items-center justify-center px-8 py-4 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-semibold shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 hover:-translate-y-0.5 transition-all duration-200">
          ...
          <>
              ...
              {etablissements.map((etab, index) => {
                ...
                return (
                  <article
                    ...
                    className="group relative h-[340px] rounded-2xl overflow-hidden bg-cover bg-center shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                      <h3 className="text-xl font-bold mb-1 line-clamp-1">{etab.nom || "Nom inconnu"}</h3>
                      <p className="text-sm opacity-90 mb-2">{etab.type || "Non spécifié"}</p>
                      {etab.description && (
                      ...
                      <Link to={`/details/${etab.id}`} onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-2 text-emerald-300 hover:text-emerald-200 font-semibold text-sm transition-colors">
                        Voir les détails
            ...
            <div className="mt-12 pt-8 border-t border-slate-200 dark:border-slate-700 text-center">
              <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">
                <strong>Astuce :</strong> Vous pouvez retirer un établissement de vos favoris en cliquant à nouveau sur le cœur dans la page des détails.
              </p>
              {etablissements.length > 0 && (
                <div className="flex flex-wrap justify-center gap-3">
                  <button onClick={() => navigate("/etablissements")} className="px-6 py-3 rounded-xl border-2 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-semibold hover:bg-emerald-500 hover:text-white transition-colors">
                    Voir plus d&apos;établissements
                  </button>
                  <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="px-6 py-3 rounded-xl bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
                    Retour en haut
                  </button>
                </div>
              )}
